...
# Local open-source models (no keys needed)
from sentence_transformers import SentenceTransformer
from transformers import pipeline

st.set_page_config(page_title="HR GenAI Copilot (Consumer)", page_icon="🛍️", layout="wide")
...
def load_data():
    ...
    # tickets may be empty; ensure columns exist
...
def top_match(query: str, texts, emb, top_k=1):
    ...
    sims = cosine_similarity([qv[0]], emb)[0]  # 1 x N
...
def t5_polish(text: str, tag: str, enabled: bool = True):
    if not enabled:
    ...
    try:
        out = t5(prompt, max_length=120, do_sample=False)
...
#st.caption("RAG with Sentence-Transformers + optional FLAN-T5 polish. No paid API.")

with st.sidebar:
    st.header("🔧 Actions")
    mode = st.radio("Select mode", ["Chatbot Q&A", "Employee Lookup", "Letter Generator", "Customer Support", "Escalations"])
    st.markdown("---")
    use_t5 = st.toggle("Use FLAN-T5 polish", value=True)
...
if mode == "Chatbot Q&A":
    ...
    if st.button("Search") and query.strip():
        ...
        if s_faq >= s_pol and s_faq > 0.25:
            qa = faqs.iloc[i_faq]
            raw = qa["answer"]
            final = t5_polish(raw, tag="FAQ", enabled=use_t5)
            st.success(final)
            ...
            raw = pr["content"]
            final = t5_polish(raw, tag=pr['topic'], enabled=use_t5)
            st.success(final)
    ...
    with tab1:
        ...
        if st.button("Track Order"):
            ...
            if not row.empty:
                ...
                st.success(t5_polish(msg, tag="ORDER", enabled=use_t5))
                if r['carrier']:
    ...
    with tab2:
        ...
        if st.button("Check Availability"):
            ...
            if not df.empty:
                ...
                st.success(t5_polish(msg, tag="INVENTORY", enabled=use_t5))
